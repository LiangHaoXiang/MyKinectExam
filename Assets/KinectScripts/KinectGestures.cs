...
#region LHX我新添加的
        ...
        Left,
        Right,
        turnPageRightByLeftHand,    //自定义用左手向右更换物体手势
        turnPageLeftByLeftHand,
#endregion
	...
	public virtual void CheckForGesture(long userId, ref GestureData gestureData, float timestamp, ref Vector3[] jointsPos, ref bool[] jointsTracked)
	{
		...
		switch(gestureData.gesture)
		{
            ...
            #region   LHX 我新添加的手势
            //自定义预备姿势的上升手势
            case Gestures.turn_Up:
                MyBaseGestureListener myCubeListenerUp = GameObject.Find("KinectController").GetComponent<MyBaseGestureListener>();
                //myCubeListener.myNewGestureInfo.GetComponent<GUIText>().text = "请做好预备姿势（左手高于左手肘）";
                switch (gestureData.state)
                {
                    case 0:
                        //若追踪到左手节点和左手肘节点  左手节点高于左手肘节点 并且  左手x方向 > 左手肘，则已准备预备姿势
                        if (jointsTracked[leftHandIndex] && jointsTracked[leftElbowIndex] &&
                            jointsPos[leftHandIndex].y > jointsPos[leftElbowIndex].y &&
                            jointsPos[leftHandIndex].x > jointsPos[leftElbowIndex].x)
                        {
                            //myCubeListenerUp.myNewGestureInfo.GetComponent<GUIText>().text = "你的预备姿势为 向上";
                            //若追踪到右手节点和右手肘节点 并且 此时右手节点在右手肘节点下方，则开始进一步识别
                            if (jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] &&
                                jointsPos[rightHandIndex].y <= jointsPos[rightElbowIndex].y)
                            {
                                ...
                                //-------------------------------------------
                                //myCubeListenerUp.myNewGestureInfo.GetComponent<GUIText>().text = "好，接下来请用右手控制物体向上转动";
                            }
                        }
                        break;
                    case 1:
                        if ((timestamp - gestureData.timestamp) < 1.5f)
                        {
                            //若右手节点高于右手肘与右肩膀之间的中心点，则认为识别成功
                            bool isInPose = jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[rightShoulderIndex] &&
                                jointsPos[rightHandIndex].y >= ((jointsPos[rightElbowIndex].y + jointsPos[rightShoulderIndex].y) * 0.5) &&
                            ...
                            if (isInPose)
                            {
                                Vector3 jointPos = jointsPos[gestureData.joint];
                                CheckPoseComplete(ref gestureData, timestamp, jointPos, isInPose, 0f);
                                //myCubeListenerUp.myNewGestureInfo.GetComponent<GUIText>().text = "右手上升了";
                            }
                        }
                        else
                        {
                ...
                break;
            //自定义预备姿势的下降手势
            case Gestures.turn_Down:
                MyBaseGestureListener myCubeListenerDown = GameObject.Find("KinectController").GetComponent<MyBaseGestureListener>();
                switch (gestureData.state)
                {
                    case 0:
                        //若追踪到左手节点和左手肘节点 并且做好预备姿势时，即左手节点低于左手肘节点  并且  左手节点x方向 > 左手肘 ，则已准备预备姿势
                        if (jointsTracked[leftHandIndex] && jointsTracked[leftElbowIndex] &&
                            jointsPos[leftHandIndex].y < jointsPos[leftElbowIndex].y &&
                            jointsPos[leftHandIndex].x > jointsPos[leftElbowIndex].x)
                        {
                            //myCubeListenerDown.myNewGestureInfo.GetComponent<GUIText>().text = "你的预备姿势为 向下";
                            //若追踪到右手、右手肘、右肩膀节点 并且 右手高于右手肘与右肩膀的中心点，则开始进一步识别
                            if (jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[rightShoulderIndex] &&
                               jointsPos[rightHandIndex].y > ((jointsPos[rightElbowIndex].y + jointsPos[rightShoulderIndex].y) * 0.5f))
                            {
                                SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                                gestureData.progress = 0.5f;
                                //-------------------------------------------
                                //myCubeListenerDown.myNewGestureInfo.GetComponent<GUIText>().text = "好，接下来请用右手控制物体向下转动";
                            }
                        }
                        break;
                    case 1:
                        if ((timestamp - gestureData.timestamp) < 1.5f)
                        {
                            ...
                            if (isInPose)
                            {
                                Vector3 jointPos = jointsPos[gestureData.joint];
                                CheckPoseComplete(ref gestureData, timestamp, jointPos, isInPose, 0f);
                                //myCubeListenerDown.myNewGestureInfo.GetComponent<GUIText>().text = "右手下降了";
                            }
                        }
                        else
                        {
            ...
            case Gestures.turn_Left:
                MyBaseGestureListener myCubeListenerLeft = GameObject.Find("KinectController").GetComponent<MyBaseGestureListener>();
                switch (gestureData.state)
                {
                    case 0:
                        //若检测到预备姿势即 左手节点x方向 < 左手肘 < 左肩膀 ， 并且 左手节点低于左手肘  则已准备预备姿势
                        if (jointsTracked[leftHandIndex] && jointsTracked[leftElbowIndex] && jointsTracked[leftShoulderIndex] && 
                            jointsPos[leftHandIndex].x < jointsPos[leftElbowIndex].x && jointsPos[leftElbowIndex].x < jointsPos[leftShoulderIndex].x &&
                            jointsPos[leftHandIndex].y < jointsPos[leftElbowIndex].y) 
                        {
                            //myCubeListenerLeft.myNewGestureInfo.GetComponent<GUIText>().text = "你的预备姿势为 向左";
                            //若右手节点x方向 > 右手肘 > 右肩膀   并且  右手节点高于右手肘  ，则开始进一步识别
                            if (jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[shoulderCenterIndex] &&
                                jointsPos[rightHandIndex].x > jointsPos[rightElbowIndex].x && jointsPos[rightElbowIndex].x > jointsPos[rightShoulderIndex].x &&
                                jointsPos[rightHandIndex].y > jointsPos[rightElbowIndex].y)
                            {
                                SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                                gestureData.progress = 0.1f;
                                //-------------------------------------------
                                //myCubeListenerLeft.myNewGestureInfo.GetComponent<GUIText>().text = "好，接下来请用右手控制物体向左转动";
                            }
                    ...
                    case 1:
                        if ((timestamp - gestureData.timestamp) < 1.5f)
                        {
                            //若右手节点x方向 < 右手肘 且 右手节点x方向 < 肩膀中心点x坐标  ，则认为识别成功
                            bool isInPose = jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[shoulderCenterIndex] &&
                                    jointsPos[rightHandIndex].x < jointsPos[rightElbowIndex].x && jointsPos[rightHandIndex].x < jointsPos[shoulderCenterIndex].x;
            ...
            case Gestures.turn_Right:
                MyBaseGestureListener myCubeListenerRight = GameObject.Find("KinectController").GetComponent<MyBaseGestureListener>();
                switch (gestureData.state)
                {
                    case 0:
                        //若右手节点x方向 > 右手手肘   并且   右手节点低于右手肘   ，则已准备预备姿势
                        if (jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] &&
                            jointsPos[rightHandIndex].x > jointsPos[rightElbowIndex].x &&
                            jointsPos[rightHandIndex].y < jointsPos[rightElbowIndex].y)
                        {
                            //若左手节点x方向 < 左手肘 < 左肩膀  且  左手节点高于左手肘  ，则开始进一步识别
                            if (jointsTracked[leftHandIndex] && jointsTracked[leftElbowIndex] && jointsTracked[leftShoulderIndex] &&
                               jointsPos[leftHandIndex].x < jointsPos[leftElbowIndex].x && jointsPos[leftElbowIndex].x < jointsPos[leftShoulderIndex].x &&
                               jointsPos[leftHandIndex].y > jointsPos[leftElbowIndex].y)
                            {
                                ...
                                gestureData.progress = 0.1f;
                                //-------------------------------------------
                                //myCubeListenerRight.myNewGestureInfo.GetComponent<GUIText>().text = "好，接下来请用左手控制物体向右转动";
                            }
                        }
                        break;
                    ...
                    case 1:
                        if ((timestamp - gestureData.timestamp) < 1.5f)
                        {
                            //若左手节点x方向 > 左手肘 且 左手节点x方向 > 肩膀中心点x坐标  ，则认为识别成功
                            bool isInPose = jointsTracked[leftHandIndex] && jointsTracked[leftElbowIndex] && jointsTracked[shoulderCenterIndex] &&
                                    jointsPos[leftHandIndex].x > jointsPos[leftElbowIndex].x && jointsPos[leftHandIndex].x > jointsPos[shoulderCenterIndex].x;
            ...
            //自定义右手上升手势
            case Gestures.Up:
                MyBaseGestureListener myCubeListenerUp1 = GameObject.Find("KinectController").GetComponent<MyBaseGestureListener>();
                //myCubeListener.myNewGestureInfo.GetComponent<GUIText>().text = "请做好预备姿势（左手高于左手肘）";
                switch (gestureData.state)
                {
                    case 0:
                        //若右手节点在右手肘节点下方，右手x方向 > 肩膀中心与右肩膀之间的中心点x坐标，则开始进一步识别
                        if (jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[shoulderCenterIndex] && jointsTracked[rightShoulderIndex] &&
                            jointsPos[rightHandIndex].y < jointsPos[rightElbowIndex].y &&
                            jointsPos[rightHandIndex].x > (jointsPos[shoulderCenterIndex].x + jointsPos[rightShoulderIndex].x) * 0.5f)
                        {
                            SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                            gestureData.progress = 0.5f;
                            //-------------------------------------------
                           // myCubeListenerUp1.myNewGestureInfo.GetComponent<GUIText>().text = "好，接下来请用右手控制物体向上转动";
                        }
                        break;
                    case 1:
                        if ((timestamp - gestureData.timestamp) < 1.5f)
                        {
                            ...
                            {
                                Vector3 jointPos = jointsPos[gestureData.joint];
                                CheckPoseComplete(ref gestureData, timestamp, jointPos, isInPose, 0.0f);
                                //myCubeListenerUp1.myNewGestureInfo.GetComponent<GUIText>().text = "右手上升了";
                            }
                        }
                        else
                        {
            ...
            //自定义右手下降手势
            case Gestures.Down:
                MyBaseGestureListener myCubeListenerDown1 = GameObject.Find("KinectController").GetComponent<MyBaseGestureListener>();
                switch (gestureData.state)
                {
                    case 0:
                        //若追踪到右手、右手肘、右肩膀节点 并且 右手高于右手肘与右肩膀的中心点  并且  右手x方向 > 肩膀中心与右肩膀之间的中心点x坐标，则开始进一步识别
                        if (jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[rightShoulderIndex] && jointsTracked[shoulderCenterIndex] && 
                           jointsPos[rightHandIndex].y > ((jointsPos[rightElbowIndex].y + jointsPos[rightShoulderIndex].y) * 0.5f) &&
                           jointsPos[rightHandIndex].x > ((jointsPos[shoulderCenterIndex].x + jointsPos[rightShoulderIndex].x) * 0.5f))
                        {
                            SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                            gestureData.progress = 0.5f;
                            //-------------------------------------------
                            //myCubeListenerDown1.myNewGestureInfo.GetComponent<GUIText>().text = "好，接下来请用右手控制物体向下转动";
                        }
                        break;
                    case 1:
                        if ((timestamp - gestureData.timestamp) < 1.5f)
                        {
                            ...
                            if (isInPose)
                            {
                                Vector3 jointPos = jointsPos[gestureData.joint];
                                CheckPoseComplete(ref gestureData, timestamp, jointPos, isInPose, 0.0f);
                                //myCubeListenerDown1.myNewGestureInfo.GetComponent<GUIText>().text = "右手下降了";
                            }
                        }
                        else
                        {
            ...
            //自定义右手向左 手势
            case Gestures.Left:
                MyBaseGestureListener myCubeListenerLeft1 = GameObject.Find("KinectController").GetComponent<MyBaseGestureListener>();
                switch (gestureData.state)
                {
                    case 0:
                        //若右手节点x方向 > 右手肘 > 右肩膀   并且  右手节点高于右手肘  ，则开始进一步识别
                            ...
                            jointsPos[rightHandIndex].y > jointsPos[rightElbowIndex].y)
                        {
                            ...
                            gestureData.progress = 0.1f;
                            //-------------------------------------------
                            //myCubeListenerLeft1.myNewGestureInfo.GetComponent<GUIText>().text = "好，接下来请用右手控制物体向左转动";
                        }
                        break;
                    case 1:
                        if ((timestamp - gestureData.timestamp) < 1.5f)
                        {
                            //若右手节点x方向 < 右手肘 且 右手节点x方向 < 肩膀中心点和右肩膀之间的中点x坐标  ，则认为识别成功
                            bool isInPose = jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] &&
                                    jointsTracked[shoulderCenterIndex] && jointsTracked[rightShoulderIndex] &&
                                    jointsPos[rightHandIndex].x < jointsPos[rightElbowIndex].x &&
                                    jointsPos[rightHandIndex].x < (jointsPos[shoulderCenterIndex].x + jointsPos[rightShoulderIndex].x) * 0.5f;
            ...
            //自定义右手向右 手势
            case Gestures.Right:
                MyBaseGestureListener myCubeListenerRight1 = GameObject.Find("KinectController").GetComponent<MyBaseGestureListener>();
                switch (gestureData.state)
                {
                    case 0:
                        //若右手节点x方向 < 右手肘 且 右手节点x方向 < 肩膀中心点  且  右手节点高于右手肘  ，则开始进一步识别
                        if (jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[shoulderCenterIndex] &&
                           jointsPos[rightHandIndex].x < jointsPos[rightElbowIndex].x && jointsPos[rightHandIndex].x < jointsPos[shoulderCenterIndex].x &&
                           jointsPos[rightHandIndex].y > jointsPos[rightElbowIndex].y)
                        {
                            ...
                            gestureData.progress = 0.1f;
                            //-------------------------------------------
                            //myCubeListenerRight1.myNewGestureInfo.GetComponent<GUIText>().text = "好，接下来请用右手控制物体向右转动";
                        }
                        break;
                    ...
                    case 1:
                        if ((timestamp - gestureData.timestamp) < 1.5f)
                        {
                            //若右手节点x方向 > 右手肘 > 肩膀中心点x坐标  ，则认为识别成功
                            bool isInPose = jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[shoulderCenterIndex] &&
                                    jointsPos[rightHandIndex].x > jointsPos[rightElbowIndex].x && jointsPos[rightElbowIndex].x > jointsPos[shoulderCenterIndex].x;
                ...
                break;
            //自定义向右更换物体手势
            case Gestures.turnPageRightByLeftHand:
                MyBaseGestureListener myCubeListenerTurnPageRight = GameObject.Find("KinectController").GetComponent<MyBaseGestureListener>();
                switch (gestureData.state)
                {
                    case 0:
                        //若左手节点x方向 < 左手肘 < 左肩膀  且  左手节点高于左手肘  ，则开始进一步识别
                           ...
                           jointsPos[leftHandIndex].y > jointsPos[leftElbowIndex].y)
                        {
                            ...
                            gestureData.progress = 0.1f;
                            //-------------------------------------------
                            //myCubeListenerTurnPageRight.myNewGestureInfo.GetComponent<GUIText>().text = "好，接下来请用左手选择物体";
                        }
                        break;
            ...
            //自定义向左更换物体手势
            case Gestures.turnPageLeftByLeftHand:
                MyBaseGestureListener myCubeListenerTurnPageLeft = GameObject.Find("KinectController").GetComponent<MyBaseGestureListener>();
                switch (gestureData.state)
                {
                    case 0:
                        //若左手节点x方向 > 左手肘 且 左手节点x方向 > 肩膀中心点x坐标  且   左手节点高于左手肘  ，则开始进一步识别
                        if (jointsTracked[leftHandIndex] && jointsTracked[leftElbowIndex] && jointsTracked[shoulderCenterIndex] &&
                           jointsPos[leftHandIndex].x > jointsPos[leftElbowIndex].x && jointsPos[leftHandIndex].x > jointsPos[leftShoulderIndex].x &&
                           jointsPos[leftHandIndex].y > jointsPos[leftElbowIndex].y)
                        {
                            SetGestureJoint(ref gestureData, timestamp, leftHandIndex, jointsPos[leftHandIndex]);
                            gestureData.progress = 0.1f;
                            //-------------------------------------------
                            //myCubeListenerTurnPageLeft.myNewGestureInfo.GetComponent<GUIText>().text = "好，接下来请用左手选择物体";
                        }
                        break;
                    case 1:
                        if ((timestamp - gestureData.timestamp) < 1.5f)
                        {
                            //若左手节点x方向 < 左手肘 < 左肩膀  ，则认为识别成功
...
//							}
			...
			case Gestures.SwipeDown:
				...
				break;
#region 注释 RightHandCursor和LeftHandCursor
